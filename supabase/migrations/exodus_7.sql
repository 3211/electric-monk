...
-- Ensure uuid-ossp extension is available for deterministic UUID generation
...
VALUES
    (uuid_generate_v5(uuid_ns_dns(), 'holy-server-rack'), 'The Holy Server Rack', '+50 Sacred Acres', '🖥', 'acres_bonus', '{"acres_bonus": 50}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'golden-compiler'), 'The Golden Compiler', '+100% Gold generation', '✨', 'gold_double', '{"gold_multiplier": 2}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'eternal-patch'), 'The Eternal Patch', '+100% Food generation', '🌾', 'food_double', '{"food_multiplier": 2}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'obsidian-bible'), 'The Obsidian Bible', '+100% Heresy generation', '📕', 'heresy_double', '{"heresy_multiplier": 2}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'iron-rosary'), 'The Iron Rosary', '+50% Crusade defense', '📿', 'crusade_defense_bonus', '{"bonus_percent": 50}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'sacred-firewall'), 'The Sacred Firewall', 'Immune to Plagues', '🛡', 'plague_immunity', '{"immune": true}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'daemon-core'), 'The Daemon Core', '+100% Dogma generation', '🔥', 'dogma_double', '{"dogma_multiplier": 2}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'papal-buffer'), 'The Papal Buffer', '-50% all upkeep costs', '📛', 'upkeep_reduction', '{"reduction_percent": 50}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'null-pointer-relic'), 'The Null Pointer Relic', '+100% Dogma AND Heresy generation', '💥', 'dual_research_double', '{"dogma_multiplier": 2, "heresy_multiplier": 2}'::jsonb, true),
    (uuid_generate_v5(uuid_ns_dns(), 'shroud-of-turing'), 'The Shroud of Turing', '+100% Mana generation', '📜', 'mana_double', '{"mana_multiplier": 2}'::jsonb, true)
ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
    description = EXCLUDED.description,
    emoji_icon = EXCLUDED.emoji_icon,
...
VALUES
    (uuid_generate_v5(uuid_ns_dns(), 'gilded-path-relations'), 'gilded_path', 'black_tribunal', 'holy_way', 'final_watch', 'Needs Mana to fuel defenses; The Tribunal steals Gold.', 'Mutual economic dependency — Gilded provides Gold, Holy provides Mana.', 'Neither ally nor foe; pragmatic coexistence.'),
    (uuid_generate_v5(uuid_ns_dns(), 'holy-way-relations'), 'holy_way', 'final_watch', 'gilded_path', 'black_tribunal', 'The Watch blocks their ascension; physical stability clashes with pure Mana worship.', 'Gilded Path funds their ascetic lifestyle with Gold.', 'The Tribunal ignores pure monks; no economic overlap.'),
    (uuid_generate_v5(uuid_ns_dns(), 'final-watch-relations'), 'final_watch', 'holy_way', 'black_tribunal', 'gilded_path', 'Physical stability clashes with pure Mana worship; the Watch sees asceticism as weakness.', 'The Tribunal serves as an aggressive police force the Watch respects.', 'Pragmatic coexistence; no direct conflict or benefit.'),
    (uuid_generate_v5(uuid_ns_dns(), 'black-tribunal-relations'), 'black_tribunal', 'gilded_path', 'final_watch', 'holy_way', 'High Heresy attracts Gold hoarders; natural enemies.', 'The Tribunal serves as an aggressive police force; the Watch is their enforcer.', 'Pure monks are beneath the Tribunal''s notice.')
