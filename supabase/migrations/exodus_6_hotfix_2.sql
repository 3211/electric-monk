...
-- Description: Restores the full 9-phase calculate_automated_karma heartbeat from exodus_1, 
-- while permanently removing the fatal updated_at column call in Phase 1.
...
BEGIN
    ...
    FOR r IN SELECT p.* FROM prayers p WHERE p.is_praying = true LOOP
        ...
        IF v_new_cycles > 0 THEN
            -- CRITICAL FIX: removed updated_at = now() here to prevent transaction rollback
            UPDATE prayers
