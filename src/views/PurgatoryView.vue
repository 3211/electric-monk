    ...
    <div class="app-frame flex min-h-screen items-center justify-center px-4 py-10 sm:py-14">
      ...
      <div v-if="intercessoryCount > 0" class="glass-panel glass-panel-soft glass-gloss evil-purgatory-support mb-5 rounded-lg border border-theme-accent/20 p-4 sm:rounded-lg">
        <p class="text-sm font-medium text-theme-accent">
          üïØÅEÅE{{ intercessoryCount }} {{ intercessoryCount === 1 ? 'person is' : 'people are' }} praying for your redemption
        </p>
      ...
      <!-- Active Siege Status (read-only while banned) -->
      <div v-if="combat.activeCombats.length > 0" class="glass-panel glass-panel-strong glass-gloss evil-purgatory-indulgence mb-6 p-6 sm:p-7">
        <h2 class="text-3xl font-semibold text-theme-accent-light">Active Siege</h2>
        <p class="mt-2 text-sm text-theme-text-dim">
          You have an ongoing siege. Manage it from the Vatican when you're free.
        </p>
        ...
        <div class="mt-4 space-y-3">
          ...
          >
            ...
            <div class="mt-2 flex gap-2">
              <button v-if="c.is_attacker" @click="handleCancelSiege(c.session_id)" class="btn-secondary px-3 py-1 text-xs">
                <span class="relative z-10 font-medium">Withdraw</span>
              </button>
              <button v-if="!c.is_attacker" @click="handleSurrenderSiege(c.session_id)" class="btn-danger px-3 py-1 text-xs">
                <span class="relative z-10 font-medium">Surrender</span>
              </button>
            </div>
          </div>
        </div>
...
onMounted(async () => {
  ...
  // Start polling active combats for siege status
  combat.startPolling()
})
