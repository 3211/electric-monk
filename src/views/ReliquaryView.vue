  ...
  <div>
        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div class="flex flex-wrap items-center justify-start gap-3 xl:justify-end">
            ...
            <div v-if="economy.synodId" class="chip status-chip gap-2 px-4 py-2 text-sm">
              <span>笞費ｸ・Synod Steal Available</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <main class="app-frame py-8 lg:py-10">
      ...
      <div v-if="relics.loading" class="glass-panel glass-panel-soft p-12 text-center">
        <div class="text-4xl mb-4" style="animation: ritual-breathe 3s ease-in-out infinite">笨ｨ</div>
          ...
          >
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              ...
              <div class="rounded-md border border-theme-border/50 bg-theme-panel/30 p-2.5 text-center">
                ...
                <div class="text-sm font-semibold text-yellow-500">{{ relic.steal_cost || 50 }} 腸</div>
              </div>
            </div>
            ...
            <div class="rounded-lg border border-theme-border/50 bg-theme-panel/30 p-3 mb-4">
              ...
              <div v-else class="text-center text-sm text-theme-text-muted py-1">
                笨ｦ Unclaimed 窶・Free for the taking
              </div>
            </div>
            ...
            <div v-if="relic.holder_id && relic.holder_id !== currentUserId && isRelicFromSynodMember(relic)" class="rounded-md border border-theme-accent/30 bg-theme-accent/5 p-2 mb-4 text-center">
              <span class="text-xs text-theme-accent font-medium">笞費ｸ・Synod Buff Active</span>
            </div>
            ...
            >
              <span class="relative z-10 font-medium">
                {{ !economy.synodId ? 'Requires Synod' : (relics.stealing ? 'Stealing...' : '笞費ｸ・Attempt Steal') }}
              </span>
            </button>
            <div v-else class="text-center py-2">
              <span class="chip status-chip text-xs">笨・In Your Possession</span>
            </div>
          </div>
        </div>
          ...
          <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="rounded-lg border border-theme-border bg-theme-panel/40 p-5">
              ...
              <div v-if="indulgences.hasPapalBull" class="mb-3">
                <span class="chip status-chip text-xs">笨・Active 窶・{{ indulgences.papalBullRemaining }} remaining</span>
              </div>
              ...
              <div v-else class="text-center">
                <span class="text-xs text-green-600 font-medium">孱・・Protected</span>
              </div>
            </div>
            ...
            <div class="rounded-lg border border-theme-border bg-theme-panel/40 p-5">
              ...
              <div v-if="indulgences.hasDivineArchitect" class="mb-3">
                <span class="chip status-chip text-xs">笨・Active</span>
              </div>
              <div v-else class="mb-3">
                <p class="text-xs text-theme-text-muted">Cost: 1 Indulgence</p>
              </div>
              ...
              <div v-else class="text-center">
                <span class="text-xs text-green-600 font-medium">笞｡ Building</span>
              </div>
            </div>

            <!-- Indulgence Balance -->
            <div class="rounded-lg border border-theme-accent/20 bg-theme-accent/5 p-5 text-center">
              <div class="text-4xl mb-2">笨ｨ</div>
